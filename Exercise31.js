...
let current_users = ["Kamran Tessori", "Zia Khan", "Daniyal Nagori", "Ameen Alam", "Bilal Fareed"];
let new_users = ["Kamran Tessori", "Zia Khan", "Anas Saleem ", "Osama Saleem", "Anmol Saleem"];
for (let i = 0; i < new_users.length; i++) {
    let new_name = true;
    for (let j = 0; j < current_users.length; j++) {
        if (new_users[i] === current_users[j]) {
            console.log(new_users[i] + " -This username has already been used");
            new_name = false;
            break;
        }
    }
